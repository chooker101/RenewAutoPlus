    ...
    public void specialAttack(PlayerEntity player, Entity target, Hand hand) {
        ...
        if ((f *= 0.2f + h * h * 0.8f) > 0.0f || g > 0.0f) {
            ...
            if (player.isSprinting() && bl) {
                player.method_48926().playSound(null, player.getX(), player.getY(), player.getZ(), SoundEvents.ENTITY_PLAYER_ATTACK_KNOCKBACK, player.getSoundCategory(), 1.0f, 1.0f);
                ++i;
                bl2 = true;
            }
            ...
            boolean bl6 = target.damage(player.method_48926().getDamageSources().playerAttack(player), f);
            if (bl6) {
                ...
                if (bl42) {
                    ...
                    list = player.method_48926().getNonSpectatingEntities(LivingEntity.class, target.getBoundingBox().expand(1.0, 0.25, 1.0));
                    ...
                    while (iterator.hasNext()) {
                        ...
                        livingEntity.damage(player.method_48926().getDamageSources().playerAttack(player), k);
                    }
                ...
                if (bl3) {
                    player.method_48926().playSound(null, player.getX(), player.getY(), player.getZ(), SoundEvents.ENTITY_PLAYER_ATTACK_CRIT, player.getSoundCategory(), 1.0f, 1.0f);
                    player.addCritParticles(target);
                }
                if (!bl3 && !bl42) {
                    if (bl) {
                        player.method_48926().playSound(null, player.getX(), player.getY(), player.getZ(), SoundEvents.ENTITY_PLAYER_ATTACK_STRONG, player.getSoundCategory(), 1.0f, 1.0f);
                    } else {
                        player.method_48926().playSound(null, player.getX(), player.getY(), player.getZ(), SoundEvents.ENTITY_PLAYER_ATTACK_WEAK, player.getSoundCategory(), 1.0f, 1.0f);
                    }
                }
                if (g > 0.0f) {
                ...
                if (!player.method_48926().isClient && !k.isEmpty() && entity4 instanceof LivingEntity) {
                ...
                if (target instanceof LivingEntity) {
                    ...
                    if (player.method_48926() instanceof ServerWorld && l > 2.0f) {
                        int livingEntity = (int)((double)l * 0.5);
                        ((ServerWorld)player.method_48926()).spawnParticles(ParticleTypes.DAMAGE_INDICATOR, target.getX(), target.getBodyY(0.5), target.getZ(), livingEntity, 0.1, 0.0, 0.1, 0.2);
                    }
                }
                player.addExhaustion(0.1f);
            } else {
                player.method_48926().playSound(null, player.getX(), player.getY(), player.getZ(), SoundEvents.ENTITY_PLAYER_ATTACK_NODAMAGE, player.getSoundCategory(), 1.0f, 1.0f);
                if (bl5) {
                    target.extinguish();
                }
            }
