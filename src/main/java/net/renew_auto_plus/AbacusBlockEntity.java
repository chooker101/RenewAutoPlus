...
public class AbacusBlockEntity extends LockableContainerBlockEntity implements ExtendedScreenHandlerFactory {
    ...
    private int autoTradeDelay = 60;
    private int currentAutoTradeDelay = 0;
    ...
    public int getAutoTradeDelay() {
        return autoTradeDelay;
    }

    public int getCurrentAutoTradeDelay() {
        return currentAutoTradeDelay;
    }

    public void setCurrentAutoTradeDelay(int i) {
        currentAutoTradeDelay = i;
    }
    ...
    public static void tick(World world, BlockPos pos, BlockState state, AbacusBlockEntity blockEntity) {
        int i = blockEntity.currentAutoTradeDelay + 1;
        if(i >= blockEntity.autoTradeDelay)
        {
            blockEntity.tryTransactAutoTrade(blockEntity.currentAutoTradeIndex);
            blockEntity.iterateAutoTradeIndex();
            if((blockEntity.currentAutoTradeIndex + 1) >= blockEntity.autoTradeList.size()) {
                blockEntity.currentAutoTradeDelay = 0;
            }
        }
        else {
            blockEntity.currentAutoTradeDelay = i;
        }
        return;
    }

    public void sendUpdateStallTradeList(){
        PacketByteBuf packetByteBuf = new PacketByteBuf(Unpooled.buffer());
        tryToAttachToMarket();
        packetByteBuf.writeBoolean(attachedMarket != null);
        if(attachedMarket != null) {
            packetByteBuf.writeByte(STALL_TRADE_LIST_TYPE);
            getRealEmeraldAmount();
        ...
        for (ServerPlayerEntity player : PlayerLookup.tracking(this)) {
            ServerPlayNetworking.send(player, RenewAutoPlusInitialize.ABACUS_SCREEN_LIST_PACKET_ID, packetByteBuf);
        }
    }

    public void sendUpdateOwnerNameList(){
        PacketByteBuf packetByteBuf = new PacketByteBuf(Unpooled.buffer());
        packetByteBuf.writeBoolean(true);
        packetByteBuf.writeByte(OWNER_NAME_LIST_TYPE);
    ...
    public void sendUpdateAttachedCrateList(){
        PacketByteBuf packetByteBuf = new PacketByteBuf(Unpooled.buffer());
        packetByteBuf.writeBoolean(true);
        packetByteBuf.writeByte(ATTACHED_CRATES_LIST_TYPE);
    ...
    public void sendUpdateAutoTradeList(){
        PacketByteBuf packetByteBuf = new PacketByteBuf(Unpooled.buffer());
        packetByteBuf.writeBoolean(true);
        packetByteBuf.writeByte(AUTO_TRADE_LIST_TYPE);
        packetByteBuf.writeByte((byte)(autoTradeList.size() & 0xFF));
        for(AutoStallTrade trade : autoTradeList) {
            trade.setAutoPurchase(trade.isPurchase);
